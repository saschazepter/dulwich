...
class cmd_diff(Command):
    def run(self, args) -> None:
        parser = argparse.ArgumentParser()
        parser.add_argument(
            "commitish", nargs="*", default=[], help="Commits or refs to compare"
        )
        ...
        if len(args.commitish) == 0:
            ...
            porcelain.diff(
                ".",
                commit=args.commitish[0],
                staged=False,
                paths=args.paths or None,
                outstream=sys.stdout.buffer,
            )
        elif len(args.commitish) == 2:
            # Show diff between two commits
            porcelain.diff(
                ".",
                commit=args.commitish[0],
                commit2=args.commitish[1],
                paths=args.paths or None,
                outstream=sys.stdout.buffer,
            )
        else:
