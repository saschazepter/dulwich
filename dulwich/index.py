...
def build_index_from_tree(
    ...
    # Validate up front so an invalid entry aborts before anything is written.
    ...
    *,
    ...
    for change in changes:
        if change.type in (
            ...
            assert change.new is not None and change.new.path is not None
            if not validate_path(change.new.path, validate_path_element):
                raise InvalidPathError(change.new.path)
