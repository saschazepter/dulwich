...
class TestUpdateWorkingTree(TestCase):
    ...
    def test_update_working_tree_ignore_case_parameter(self):
        """Test that ignore_case parameter overrides configuration detection."""
        # Create tree with lowercase file
        blob1 = Blob()
        blob1.data = b"lowercase content"
        self.repo.object_store.add_object(blob1)
        ...
        # Test with ignore_case=True (simulate case-insensitive behavior)
        changes = tree_changes(self.repo.object_store, tree1.id, tree2.id)
        update_working_tree(
            self.repo, tree1.id, tree2.id, change_iterator=changes, ignore_case=True
        )

        # Should behave as if on case-insensitive filesystem
        lowercase_path = os.path.join(self.tempdir, "readme.txt")
        uppercase_path = os.path.join(self.tempdir, "README.txt")

        # On a case-sensitive filesystem with ignore_case=True, should still create both files
        # but the old file should not be deleted (case-only rename behavior)
        self.assertTrue(os.path.exists(uppercase_path))
        with open(uppercase_path, "rb") as f:
            self.assertEqual(b"uppercase content", f.read())
        ...
        # Test with ignore_case=False (force case-sensitive behavior)
        changes = tree_changes(self.repo.object_store, tree1.id, tree2.id)
        update_working_tree(
            self.repo, tree1.id, tree2.id, change_iterator=changes, ignore_case=False
        )

        # Should behave as case-sensitive (delete old, create new)
        # However, on case-insensitive filesystems, the operating system
        # will map both paths to the same file
        import platform

        # Check what actually happened based on filesystem behavior
        # With ignore_case=False, should behave case-sensitively
        # On case-insensitive filesystems (macOS, Windows),
        # lowercase_path and uppercase_path refer to the same file
        if platform.system() in ("Darwin", "Windows"):
            # On case-insensitive filesystems, when ignore_case=False,
            # the operation tries to delete readme.txt and create README.txt.
            # But since they refer to the same file, the result is that
            # the file exists with the new name and content.
            # Add debugging info
            # On case-insensitive FS, the file should exist and be accessible
            # via either path, with the new content
            file_exists = os.path.exists(uppercase_path) or os.path.exists(
                lowercase_path
            )
            self.assertTrue(
                file_exists,
                f"File should exist. Files in {self.tempdir}: {os.listdir(self.tempdir)}",
            )

            # Read the content - try both paths since on case-insensitive FS they're the same
            ...
            self.assertEqual(
                b"uppercase content",
                content,
                f"File content should be 'uppercase content', got {content!r}",
            )
        else:
            # On case-sensitive filesystems, old file should be deleted, new one created
            self.assertTrue(
                os.path.exists(uppercase_path),
                f"uppercase_path {uppercase_path} should exist",
            )
            self.assertFalse(
                os.path.exists(lowercase_path),
                f"lowercase_path {lowercase_path} should not exist on case-sensitive FS",
            )
            with open(uppercase_path, "rb") as f:
                content = f.read()
                self.assertEqual(
                    b"uppercase content",
                    content,
                    f"uppercase_path content should be 'uppercase content', got {content!r}",
                )
