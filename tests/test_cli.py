...
class GitFlushTest(TestCase):
    ...
    def test_text_wrapper_flushes_on_write(self):
        ...
        mock_stream = MagicMock()
        wrapper = AutoFlushTextIOWrapper(mock_stream, auto_flush=True)

        wrapper.write("test")
        mock_stream.write.assert_called_once_with("test")
        mock_stream.flush.assert_called_once()

    def test_text_wrapper_no_flush_when_disabled(self):
        """Test that AutoFlushTextIOWrapper doesn't flush when disabled."""
        from dulwich.cli import AutoFlushTextIOWrapper

        mock_stream = MagicMock()
        wrapper = AutoFlushTextIOWrapper(mock_stream, auto_flush=False)
    ...
    def test_binary_wrapper_flushes_on_write(self):
        ...
        mock_stream = MagicMock()
        wrapper = AutoFlushBinaryIOWrapper(mock_stream, auto_flush=True)
    ...
    def test_text_wrapper_env_classmethod(self):
        ...
        self.assertTrue(wrapper._auto_flush)

        wrapper = AutoFlushTextIOWrapper.env(mock_stream, env={"GIT_FLUSH": "0"})
        self.assertFalse(wrapper._auto_flush)
